  ...
  return (
    <li className='relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]'>
        <Link className='contents' to={`/category/${listing.type}/${id}`}>
            <img className='h-[170px] w-full object-cover hover:scale-105 transition-scale duration-200 ease-in' loading='lazy' src={listing.imgUrls[0]} alt=''></img>
            ...
            <p className='text-[#457b9d] mt-2 font-semibold'>
                ${listing.offer ? listing.discountPrice.toString().replace(/\B(?=(d{3})+(?!\d))/g, ",") : listing.regularPrice.toString().replace(/\B(?=(d{3})+(?!\d))/g, ",")}
